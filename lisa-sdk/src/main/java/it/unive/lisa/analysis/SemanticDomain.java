...
public interface SemanticDomain<D extends SemanticDomain<D, E, I>, E extends SymbolicExpression, I extends Identifier>
	...
	default Pair<D, D> split(
		...
		if (src.getCFG().containsEdge(new TrueEdge((Statement) src, (Statement) dest)))
			other = src.getCFG().getOutgoingEdges((Statement) src).stream().filter(e -> e instanceof FalseEdge)
					.findFirst().get().getDestination();
		else
			other = src.getCFG().getOutgoingEdges((Statement) src).stream().filter(e -> e instanceof TrueEdge)
					.findFirst().get().getDestination();
