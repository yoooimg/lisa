...
public interface BaseInferredValue<T extends BaseInferredValue<T>> extends BaseLattice<T>, InferredValue<T> {
	...
	default Pair<InferenceSystem<T>, InferenceSystem<T>> split(
			...
			throws SemanticException {
		return Pair
				.of(this.assume(environment, expression, src, dest, oracle),
						this.assume(environment, new UnaryExpression(expression.getStaticType(),
								expression, LogicalNegation.INSTANCE, expression.getCodeLocation()), src, dest,
								oracle));
	}
}
