...
public class Interval implements BaseNonRelationalValueDomain<Interval>, Comparable<Interval> {
	...
	@Override
	public Pair<ValueEnvironment<Interval>, ValueEnvironment<Interval>> split(
			ValueEnvironment<Interval> environment,
		...
		if (!(expr instanceof BinaryExpression)) {
			return Pair.of(this.assume(environment, expr, src, dest, oracle), this.assume(environment,
					new UnaryExpression(expr.getStaticType(),
							expr, LogicalNegation.INSTANCE, expr.getCodeLocation()),
					src, dest, oracle));
		}
