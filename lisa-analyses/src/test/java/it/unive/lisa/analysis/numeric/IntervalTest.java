...
public class IntervalTest {
	...
	public void testSplitOnInterval() throws SemanticException {
		...
		for (int i = 0; i < TEST_LIMIT; i++) {
			BinaryExpression eqExpr = new BinaryExpression(Int32Type.INSTANCE, x, c, ComparisonEq.INSTANCE,
					pp.getLocation());
			BinaryExpression neExpr = new BinaryExpression(Int32Type.INSTANCE, x, c, ComparisonNe.INSTANCE,
					pp.getLocation());
			BinaryExpression gtExpr = new BinaryExpression(Int32Type.INSTANCE, x, c, ComparisonGt.INSTANCE,
					pp.getLocation());
			BinaryExpression ltExpr = new BinaryExpression(Int32Type.INSTANCE, x, c, ComparisonLt.INSTANCE,
					pp.getLocation());
			BinaryExpression geExpr = new BinaryExpression(Int32Type.INSTANCE, x, c, ComparisonGe.INSTANCE,
					pp.getLocation());
			BinaryExpression leExpr = new BinaryExpression(Int32Type.INSTANCE, x, c, ComparisonLe.INSTANCE,
					pp.getLocation());

			Interval xInterval = mk(rand.nextInt(), rand.nextInt());
			int randInt = rand.nextInt();
			Interval cInterval = mk(randInt, randInt);
			ValueEnvironment<Interval> environment = new ValueEnvironment<>(xInterval).putState(x, xInterval)
					.putState(c, cInterval);
			...
			Pair<ValueEnvironment<Interval>,
					ValueEnvironment<Interval>> actualEqEnvs = xInterval.split(environment, eqExpr, pp, pp, oracle);
			Pair<ValueEnvironment<Interval>,
					ValueEnvironment<Interval>> expectedEqEnvs = Pair.of(
			...
			System.out.println("true branch ValueEnvironment: " + actualEqEnvs.getLeft());
			System.out.println("false branch ValueEnvironment: " + actualEqEnvs.getRight());
			assertEquals(actualEqEnvs, expectedEqEnvs);
			System.out.println("*******************************************");

			System.out.println("*********************(!=)*******************");
			System.out.println("expression: " + neExpr);
			Pair<ValueEnvironment<Interval>,
					ValueEnvironment<Interval>> actualNeEnvs = xInterval.split(environment, neExpr, pp, pp, oracle);
			Pair<ValueEnvironment<Interval>,
					ValueEnvironment<Interval>> expectedNeEnvs = Pair.of(
							new ValueEnvironment<>(xInterval).putState(x, xInterval).putState(c, cInterval),
							new ValueEnvironment<>(xInterval).putState(x, cInterval).putState(c, cInterval));
			System.out.println("true branch ValueEnvironment: " + actualNeEnvs.getLeft());
			System.out.println("false branch ValueEnvironment: " + actualNeEnvs.getRight());
			assertEquals(actualNeEnvs, expectedNeEnvs);
			System.out.println("********************************************");

			System.out.println("*********************(>)*******************");
			System.out.println("expression: " + gtExpr);
			Pair<ValueEnvironment<Interval>,
					ValueEnvironment<Interval>> actualGtEnvs = xInterval.split(environment, gtExpr, pp, pp, oracle);
			Pair<ValueEnvironment<Interval>, ValueEnvironment<Interval>> expectedGtEnvs = null;
			if (low.gt(constantNumber))
				expectedGtEnvs = Pair.of(
						new ValueEnvironment<>(xInterval).putState(x, xInterval).putState(c, cInterval),
				...
				expectedGtEnvs = Pair.of(new ValueEnvironment<>(xInterval)
						.putState(x, new Interval(constantNumber.add(MathNumber.ONE), high)).putState(c, cInterval),
						new ValueEnvironment<>(xInterval).putState(x, new Interval(low, constantNumber)).putState(c,
								cInterval));
			} else {
				expectedGtEnvs = Pair.of(
						new ValueEnvironment<>(xInterval).putState(x, xInterval).putState(c, cInterval),
			...
			System.out.println("true branch ValueEnvironment: " + actualGtEnvs.getLeft());
			System.out.println("false branch ValueEnvironment: " + actualGtEnvs.getRight());
			assertEquals(actualGtEnvs, expectedGtEnvs);
			System.out.println("********************************************");

			System.out.println("*********************(<)*******************");
			System.out.println("expression: " + ltExpr);
			Pair<ValueEnvironment<Interval>,
					ValueEnvironment<Interval>> actualLtEnvs = xInterval.split(environment, ltExpr, pp, pp, oracle);
			Pair<ValueEnvironment<Interval>, ValueEnvironment<Interval>> expectedLtEnvs = null;
			if (high.lt(constantNumber))
				expectedLtEnvs = Pair.of(
						new ValueEnvironment<>(xInterval).putState(x, xInterval).putState(c, cInterval),
						new ValueEnvironment<>(new Interval().bottom()));
			else if (low.lt(constantNumber) && high.geq(constantNumber)) {
				expectedLtEnvs = Pair.of(new ValueEnvironment<>(xInterval)
						.putState(x, new Interval(low, constantNumber.subtract(MathNumber.ONE))).putState(c, cInterval),
						new ValueEnvironment<>(xInterval).putState(x, new Interval(constantNumber, high)).putState(c,
								cInterval));
			} else if (high.geq(constantNumber)) {
				expectedLtEnvs = Pair.of(new ValueEnvironment<>(new Interval().bottom()),
						new ValueEnvironment<>(xInterval).putState(x, xInterval).putState(c, cInterval));
			}
			System.out.println("true branch ValueEnvironment: " + actualLtEnvs.getLeft());
			System.out.println("false branch ValueEnvironment: " + actualLtEnvs.getRight());
			assertEquals(actualLtEnvs, expectedLtEnvs);
			System.out.println("********************************************");

			System.out.println("*********************(>=)*******************");
			System.out.println("expression: " + geExpr);
			Pair<ValueEnvironment<Interval>,
					ValueEnvironment<Interval>> actualGeEnvs = xInterval.split(environment, geExpr, pp, pp, oracle);
			...
			if (constantNumber.leq(low) && constantNumber.lt(high))
				...
				expectedGeEnvs = Pair.of(
						new ValueEnvironment<>(xInterval).putState(x, new Interval(constantNumber, high)).putState(c,
								cInterval),
						new ValueEnvironment<>(xInterval)
								.putState(x, new Interval(low, constantNumber.subtract(MathNumber.ONE)))
								.putState(c, cInterval));
			...
			System.out.println("true branch ValueEnvironment: " + actualGeEnvs.getLeft());
			System.out.println("false branch ValueEnvironment: " + actualGeEnvs.getRight());
			assertEquals(actualGeEnvs, expectedGeEnvs);
			System.out.println("********************************************");

			System.out.println("*********************(<=)*******************");
			System.out.println("expression: " + leExpr);
			Pair<ValueEnvironment<Interval>,
					ValueEnvironment<Interval>> actualLeEnvs = xInterval.split(environment, leExpr, pp, pp, oracle);
			...
			if (low.gt(constantNumber) && high.gt(constantNumber))
				...
				expectedLeEnvs = Pair.of(
						new ValueEnvironment<>(xInterval).putState(x, new Interval(low, constantNumber)).putState(c,
								cInterval),
						new ValueEnvironment<>(xInterval)
								.putState(x, new Interval(constantNumber.add(MathNumber.ONE), high))
								.putState(c, cInterval));
			} else if (high.leq(constantNumber)) {
				expectedLeEnvs = Pair.of(
						new ValueEnvironment<>(xInterval).putState(x, xInterval).putState(c, cInterval),
			...
			System.out.println("true branch ValueEnvironment: " + actualLeEnvs.getLeft());
			System.out.println("false branch ValueEnvironment: " + actualLeEnvs.getRight());
			assertEquals(actualLeEnvs, expectedLeEnvs);
			System.out.println("********************************************");
		}
	}
}
